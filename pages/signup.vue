<template>
  <div class="min-h-screen bg-gray-50 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      ...
      <form class="mt-8 space-y-6" @submit.prevent="handleSignup">
        <div class="rounded-md shadow-sm space-y-4">
          ...
          <div>
            ...
            />
            <p class="mt-1 text-xs text-gray-500 font-inter">Password must be at least 6 characters long</p>
          </div>
        ...
        <div class="flex items-center">
          ...
          />
          <label for="terms" class="ml-2 block text-sm text-gray-900 font-inter">
            I agree to the
            <NuxtLink to="/contractor-agreement" class="text-amber-600 hover:text-amber-500">Terms & Conditions</NuxtLink>
            and
            <NuxtLink to="/privacy-policy" class="text-amber-600 hover:text-amber-500">Privacy Policy</NuxtLink>
          </label>
        </div>
    ...
    <!-- Phone Number Modal (if needed after Google sign-in) -->
    <div 
      v-if="showPhoneModal" 
        ...
        <form @submit.prevent="handlePhoneSubmit">
          <div class="mb-4">
            ...
            <input
              ...
              type="tel"
              required
              class="appearance-none relative block w-full px-3 py-3 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-lg focus:outline-none focus:ring-amber-500 focus:border-amber-500 sm:text-sm font-inter"
              placeholder="+44 1234 567890"
            />
          </div>
          ...
          <div class="flex gap-3">
            <button
              type="button"
              @click="skipPhone"
              class="flex-1 py-2 px-4 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 font-inter transition-colors"
            >
              Skip for now
            </button>
            <button
              type="submit"
              :disabled="!modalPhone"
              class="flex-1 py-2 px-4 bg-amber-600 text-white rounded-lg text-sm font-medium hover:bg-amber-700 disabled:opacity-50 disabled:cursor-not-allowed font-inter transition-colors"
            >
              Save
            </button>
          </div>
...
definePageMeta({
  layout: false,
  title: 'Sign Up - City Cars'
});

const { signUp, signInWithGoogle, updateUserPhone, isLoading, error } = useAuth();
...
const handleSignup = async () => {
  ...
  try {
    ...
    if (form.phone) {
      console.log('[SIGNUP] Updating phone number:', form.phone);
      await updateUserPhone(form.phone);
    }
...
const handleGoogleSignUp = async () => {
  ...
  try {
    ...
    console.log('[SIGNUP] Google sign-up result:', result);
    
    if (result && result.success) {
      console.log('[SIGNUP] Sign-up successful');
      console.log('[SIGNUP] User phone number:', result.user?.phoneNumber);
      
      // Check if user needs to add phone number
      if (!result.user.phoneNumber) {
        console.log('[SIGNUP] No phone number, showing modal');
        showPhoneModal.value = true;
      } else {
        console.log('[SIGNUP] Phone exists, redirecting to home');
        await router.push('/');
      }
    } else {
      console.error('[SIGNUP] Sign-up failed - no success flag');
    }
  } catch (err: any) {
...
const handlePhoneSubmit = async () => {
  ...
  // Redirect to phone verification page with the phone number
  await router.push({
    path: '/verify-phone',
    query: { phone: modalPhone.value, redirect: '/' }
  });
};
