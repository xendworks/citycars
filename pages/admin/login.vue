    ...
    <div class="w-full max-w-md">
      ...
      <div class="bg-white rounded-2xl shadow-xl p-8">
        ...
        <div class="mt-6 p-4 bg-amber-50 rounded-lg border border-amber-200">
          <p class="text-xs font-semibold text-amber-800 mb-1">⚠️ Setup Required</p>
          ...
          <p class="text-xs text-amber-700 mt-2">
            Or use Google Sign-in (already enabled) with an admin account.
          </p>
        </div>
      </div>
...
const handleGoogleSignIn = async () => {
  ...
  try {
    ...
    console.log('[ADMIN LOGIN] Google sign-in result:', result);
    
    // Check if user has admin role
    const { getUserProfile } = useFirestore();
    const profile = await getUserProfile(result.user.uid);
    
    if (!profile?.role || !['super_admin', 'manager', 'operator'].includes(profile.role)) {
