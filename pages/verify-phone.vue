  ...
  <div class="min-h-screen bg-gray-50 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full space-y-8">
      ...
      <div class="text-center">
        ...
        <h2 class="text-3xl font-bold text-gray-900 font-lexend">Verify Your Phone</h2>
      ...
      <!-- Success Message -->
      <div v-if="successMessage" class="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-md text-sm font-inter">
        {{ successMessage }}
      </div>
      ...
      <div v-if="step === 1" class="bg-white rounded-lg shadow-md p-6">
        <form @submit.prevent="sendCode" class="space-y-6">
          ...
          <button
            ...
            class="w-full flex justify-center py-3 px-4 border border-transparent text-sm font-medium rounded-lg text-white bg-amber-600 hover:bg-amber-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amber-500 disabled:opacity-50 disabled:cursor-not-allowed font-lexend transition-colors"
          >
      ...
      <div v-if="step === 2" class="bg-white rounded-lg shadow-md p-6">
        <div class="text-center mb-6">
          ...
          <p class="text-lg font-semibold text-gray-900 font-lexend">
            {{ phoneNumber }}
          </p>
          <button
            @click="changeNumber"
            class="mt-2 text-sm text-amber-600 hover:text-amber-700 font-inter"
          >
            Change number
          </button>
        </div>
        ...
        <form @submit.prevent="verifyCode" class="space-y-6">
          <div>
            <label for="code" class="block text-sm font-medium text-gray-700 mb-1 font-lexend">
              Verification Code
            </label>
            <input
              id="code"
...
onMounted(async () => {
  ...
  try {
    ...
    console.log('[VERIFY-PHONE] reCAPTCHA initialized');
  } catch (err) {
...
const sendCode = async () => {
  ...
  try {
    console.log('[VERIFY-PHONE] Sending code to:', phoneNumber.value);
...
const verifyCode = async () => {
  ...
  try {
    console.log('[VERIFY-PHONE] Verifying code:', verificationCode.value);
